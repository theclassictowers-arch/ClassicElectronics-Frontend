...
export const downloadInvoicePdf = async ({
  ...
  totalAmount,
      ...
      if (activeDocumentType === 'quotation') {
        ...
        const quotationMaxRowHeight = 44;
        ...
        const quotationRowHeights = quotationItems.map((item, index) => {
          ...
          const imageHeight = quotationImageDataUrls[index] ? 36 : 0;
        ...
        const drawQuotationRow = (item: InvoiceItem, index: number, rowY: number, rowHeight: number) => {
          ...
          const imageReservedHeight = itemImage ? 34 : 0;
          ...
          if (itemImage) {
            ...
            const maxImageHeight = Math.min(26, Math.max(rowHeight - (imageY - rowY) - 2, 5));
