...
export const downloadInvoicePdf = async ({
  ...
  totalAmount,
      ...
      if (activeDocumentType === 'quotation') {
        ...
        const quotationMaxRowHeight = 36;
        ...
        const quotationRowHeights = quotationItems.map((item, index) => {
          const nameLines = item.productName
            ? splitPdfCellText(item.productName, descriptionWidth - 4, 7.5)
            : [];
          ...
          const imageHeight = quotationImageDataUrls[index] ? 29 : 0;
        ...
        const drawQuotationRow = (item: InvoiceItem, index: number, rowY: number, rowHeight: number) => {
          ...
          const imageReservedHeight = itemImage ? 27 : 0;
          ...
          if (itemImage) {
            ...
            const maxImageHeight = Math.min(20.8, Math.max(rowHeight - (imageY - rowY) - 2, 5));
      ...
      for (const [index, item] of items.entries()) {
        ...
        const imageHeight = itemImage ? 17 : 0;
        ...
        if (itemImage) {
          ...
          const maxImageHeight = Math.min(17, Math.max(finalRowHeight - (imageY - cursorY) - 2, 7));
          const imageSize = containImageSize(itemImage, maxImageWidth, maxImageHeight);
